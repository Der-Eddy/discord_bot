...
class gwent():
    ...
    @commands.command(pass_context=True, aliases=['gwentcard'])
    ...
    async def gwent(self, ctx, *card: str):
        ...
        async with aiohttp.post(url) as r:
            if r.status == 200:
                ...
                if json['group'] == 'bronze':
                    color = 0xcd7f32
                elif json['group'] == 'silver':
                    color = 0xc0c0c0
                else:
                    color = 0xf1c40f #leader and gold cards
                embed = discord.Embed(color=color)
                embed.set_footer(text='https://www.playgwent.com')
